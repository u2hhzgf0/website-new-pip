    ...
    <div className="space-y-6">
      ...
      <div className="flex items-start gap-3 sm:gap-4 flex-wrap">
        ...
        <div className="flex-1 min-w-0">
          <div className="flex items-center gap-2 sm:gap-3 mb-1 flex-wrap">
            <h1 className="text-xl sm:text-2xl font-bold text-slate-900 dark:text-white">{investment.planName}</h1>
            <span className={`px-3 py-1 rounded-full text-xs font-medium border ${getStatusColor(investment.status)}`}>
              {investment.status}
            </span>
          </div>
          <p className="text-slate-500 dark:text-slate-400 text-xs sm:text-sm">Investment ID: #{investment.id}</p>
        </div>
        <Link
      ...
      <div className="bg-white dark:bg-slate-900/50 border border-slate-200 dark:border-slate-800 rounded-xl p-4 sm:p-6">
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
          ...
          <div className="bg-slate-100/50 dark:bg-slate-800/50 rounded-lg p-4">
            <p className="text-slate-500 dark:text-slate-400 text-xs mb-1">Next Profit</p>
            <div className="flex items-center gap-2">
              <Calendar className="text-emerald-500" size={16} />
              <p className="text-slate-900 dark:text-white font-semibold text-sm">{investment.nextProfitDate ? formatDate(investment.nextProfitDate) : 'N/A'}</p>
            </div>
          </div>
          <div className="bg-slate-100/50 dark:bg-slate-800/50 rounded-lg p-4">
            ...
            <div className="flex items-center gap-2">
              ...
              <p className="text-slate-900 dark:text-white font-semibold capitalize">{investment.status}</p>
            </div>
          </div>
        </div>
      </div>
          ...
          <div className="space-y-4">
            <div className="flex items-center justify-between p-4 bg-slate-100/30 dark:bg-slate-800/30 rounded-lg">
              <div>
                ...
                <p className="text-xl sm:text-2xl font-bold text-slate-900 dark:text-white mt-1">${investment.totalProfit.toLocaleString()}</p>
              </div>
            ...
            <div className="flex items-center justify-between p-4 bg-emerald-500/5 border border-emerald-500/20 rounded-lg">
              <div>
                ...
                <p className="text-xl sm:text-2xl font-bold text-emerald-400 mt-1">${investment.paidProfit.toLocaleString()}</p>
              </div>
        ...
        <div className="bg-white dark:bg-slate-900/50 border border-slate-200 dark:border-slate-800 rounded-xl p-4 sm:p-6">
          ...
          <div className="space-y-3">
            <div className="flex items-center justify-between py-3 border-b border-slate-200 dark:border-slate-800">
              <span className="text-slate-500 dark:text-slate-400 text-sm">Plan Type</span>
              <span className="text-slate-900 dark:text-white font-medium capitalize">{investment.planType}</span>
            </div>
            <div className="flex items-center justify-between py-3 border-b border-slate-200 dark:border-slate-800">
              <span className="text-slate-500 dark:text-slate-400 text-sm">Duration</span>
              <span className="text-slate-900 dark:text-white font-medium">{investment.duration} {investment.durationType}</span>
            </div>
            <div className="flex items-center justify-between py-3 border-b border-slate-200 dark:border-slate-800">
              <span className="text-slate-500 dark:text-slate-400 text-sm">Start Date</span>
              <span className="text-slate-900 dark:text-white font-medium">{formatDate(investment.startDate)}</span>
            </div>
            <div className="flex items-center justify-between py-3 border-b border-slate-200 dark:border-slate-800">
              <span className="text-slate-500 dark:text-slate-400 text-sm">End Date</span>
              <span className="text-slate-900 dark:text-white font-medium">{formatDate(investment.endDate)}</span>
            </div>
            <div className="flex items-center justify-between py-3 border-b border-slate-200 dark:border-slate-800">
              <span className="text-slate-500 dark:text-slate-400 text-sm">Last Profit</span>
              <span className="text-slate-900 dark:text-white font-medium">
                {investment.lastProfitDate ? formatDate(investment.lastProfitDate) : 'N/A'}
              </span>
            </div>
            <div className="flex items-center justify-between py-3">
              <span className="text-slate-500 dark:text-slate-400 text-sm">Total Return</span>
      ...
      <div className="bg-white dark:bg-slate-900/50 border border-slate-200 dark:border-slate-800 rounded-xl overflow-hidden">
        <div className="p-4 sm:p-6 border-b border-slate-200 dark:border-slate-800 flex flex-col sm:flex-row sm:items-center justify-between gap-2">
          <div>
            ...
            <p className="text-slate-500 dark:text-slate-400 text-xs sm:text-sm mt-1">Last 5 profit payments</p>
          </div>
          <Link
            href={`/dashboard/profits/investment/${investmentId}`}
        ...
        <div className="divide-y divide-slate-200 dark:divide-slate-800">
          {profitHistory.map((profit) => (
            <div key={profit.id} className="p-4 sm:p-6 hover:bg-slate-100/30 dark:hover:bg-slate-800/30 transition-colors">
              <div className="flex items-center justify-between">
                <div className="flex items-center gap-4">
                  ...
                  <div>
                    <p className="text-slate-900 dark:text-white font-medium">${profit.amount.toFixed(2)}</p>
                    <p className="text-slate-500 dark:text-slate-400 text-sm">{formatDateTime(profit.date)}</p>
                  </div>
                </div>
