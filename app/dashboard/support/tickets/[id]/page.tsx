...
export default function TicketDetail() {
  ...
  const getStatusColor = (status: string) => {
    const colors: Record<string, string> = {
      ...
      closed: 'text-slate-600 dark:text-slate-500 bg-slate-300/10 dark:bg-slate-500/10 border-slate-300/30 dark:border-slate-500/30',
    }
  ...
  const getPriorityColor = (priority: string) => {
    const colors: Record<string, string> = {
      low: 'text-slate-500 dark:text-slate-400 bg-slate-300/10 dark:bg-slate-500/10',
      normal: 'text-blue-400 bg-blue-500/10',
      high: 'text-rose-400 bg-rose-500/10',
      urgent: 'text-rose-400 bg-rose-500/10',
    }
    return colors[priority] || 'text-slate-500 dark:text-slate-400 bg-slate-300/10 dark:bg-slate-500/10'
  }
      ...
      <div className="space-y-6">
        <div className="flex items-center gap-4">
          ...
          <div className="flex-1">
            <h1 className="text-xl font-bold text-slate-900 dark:text-white">Loading...</h1>
          </div>
        </div>
    ...
    return (
      <div className="space-y-6">
        ...
        <div className="bg-white dark:bg-slate-900/50 border border-slate-200 dark:border-slate-800 rounded-xl p-12 text-center">
          ...
          <button
            onClick={() => router.back()}
            className="mt-4 px-4 py-2 bg-slate-100 dark:bg-slate-800 hover:bg-slate-200 dark:hover:bg-slate-700 text-slate-900 dark:text-white rounded-lg font-medium transition-colors"
          >
            Go Back
          </button>
        </div>
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex items-center gap-4">
        ...
        <div className="flex-1">
          <div className="flex items-center gap-2 sm:gap-3 mb-1 flex-wrap">
            <h1 className="text-base sm:text-xl font-bold text-slate-900 dark:text-white">{ticket.subject}</h1>
      ...
      <div className="bg-white dark:bg-slate-900/50 border border-slate-200 dark:border-slate-800 rounded-xl p-4 sm:p-6 card-lift">
        <div className="grid grid-cols-1 md:grid-cols-3 gap-4 sm:gap-6">
          <div>
            ...
            <div className="flex items-center gap-2">
              ...
              <p className="text-slate-900 dark:text-white font-medium capitalize">{ticket.status.replace('_', ' ')}</p>
            </div>
          </div>
          <div>
            <p className="text-slate-500 dark:text-slate-400 text-sm mb-1">Priority</p>
            <p className="text-slate-900 dark:text-white font-medium capitalize">{ticket.priority}</p>
          </div>
          <div>
            <p className="text-slate-500 dark:text-slate-400 text-sm mb-1">Category</p>
            <p className="text-slate-900 dark:text-white font-medium">{ticket.category}</p>
          </div>
        </div>
        ...
        {(ticket.status === 'resolved' || ticket.status === 'closed') && !ticket.rating && (
          <div className="mt-6 pt-6 border-t border-slate-200 dark:border-slate-800">
            <div className="flex items-start justify-between gap-4">
              <div>
                <h3 className="text-slate-900 dark:text-white font-medium mb-1">Rate your support experience</h3>
                <p className="text-slate-500 dark:text-slate-400 text-sm">Help us improve our service by rating this ticket</p>
              </div>
              <button
        ...
        {ticket.rating && (
          <div className="mt-6 pt-6 border-t border-slate-200 dark:border-slate-800">
            <p className="text-slate-500 dark:text-slate-400 text-sm mb-2">Your Rating</p>
            ...
            {ticket.feedback && (
              <p className="text-slate-600 dark:text-slate-300 text-sm mt-2">{ticket.feedback}</p>
            )}
          </div>
        )}
      </div>
        ...
        <div className="p-4 sm:p-6 space-y-4 sm:space-y-6 max-h-[600px] overflow-y-auto">
          {(!ticket.messages || ticket.messages.length === 0) ? (
            <div className="text-center py-8">
              <p className="text-slate-500 dark:text-slate-400">No messages yet. Send a reply to start the conversation.</p>
            </div>
          ) : (
            <>
                  ...
                  >
                    ...
                    <div className={`flex-1 max-w-2xl ${isUser ? 'items-end' : 'items-start'} flex flex-col`}>
                      <div className={`flex items-center gap-2 mb-1 ${isUser ? 'flex-row-reverse' : ''}`}>
                        ...
                        <span className="text-xs text-slate-600 dark:text-slate-500">{formatTime(msg.createdAt)}</span>
                      </div>
      ...
      {showRatingForm && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-slate-900/40 dark:bg-slate-950/80 backdrop-blur-sm p-4">
          <div className="bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-2xl max-w-md w-full p-8">
            ...
            <p className="text-slate-500 dark:text-slate-400 text-sm mb-6">How satisfied were you with the support provided?</p>
            ...
            <div className="flex gap-3">
              <button
                onClick={() => setShowRatingForm(false)}
                disabled={isRating}
                className="flex-1 py-2.5 border border-slate-300 dark:border-slate-700 text-slate-600 dark:text-slate-300 rounded-lg hover:bg-slate-100/50 dark:hover:bg-slate-800/50 transition-colors font-medium disabled:opacity-50"
              >
                Cancel
              </button>
              <button
