...
export default function MyTickets() {
  ...
  const getStatusIcon = (status: Ticket['status']) => {
    switch (status) {
      ...
      case 'closed':
        return <XCircle className="text-slate-600 dark:text-slate-500" size={16} />
    }
  }
  ...
  const getStatusBadge = (status: Ticket['status']) => {
    const styles = {
      ...
      closed: 'bg-slate-300/10 dark:bg-slate-500/10 text-slate-600 dark:text-slate-500 border-slate-300/30 dark:border-slate-500/30',
    }
    return (
  ...
  const getPriorityBadge = (priority: Ticket['priority']) => {
    const styles = {
      low: 'bg-slate-300/10 dark:bg-slate-500/10 text-slate-500 dark:text-slate-400',
    ...
    return (
      <div className="space-y-6">
        ...
        <div className="flex justify-between items-start">
          <div>
            <h1 className="text-xl sm:text-2xl font-bold text-slate-900 dark:text-white">Support Tickets</h1>
            <p className="text-slate-500 dark:text-slate-400 text-sm mt-1">View and manage your support requests</p>
          </div>
        </div>
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex flex-col sm:flex-row justify-between items-start gap-3">
        <div>
          <h1 className="text-xl sm:text-2xl font-bold text-slate-900 dark:text-white">Support Tickets</h1>
          <p className="text-slate-500 dark:text-slate-400 text-xs sm:text-sm mt-1">View and manage your support requests</p>
        </div>
        <Link
      ...
      <div className="flex flex-col md:flex-row gap-4">
        <div className="flex-1 relative">
          <Search className="absolute left-4 top-1/2 -translate-y-1/2 text-slate-600 dark:text-slate-500" size={20} />
          <input
            type="text"
          ...
          <div className="divide-y divide-slate-200 dark:divide-slate-800">
            {filteredTickets.map((ticket: Ticket) => (
              <Link
                ...
                className="block p-4 sm:p-6 hover:bg-slate-100/30 dark:hover:bg-slate-800/30 transition-colors group"
              >
                ...
                <div className="flex flex-wrap items-center gap-2 sm:gap-4 text-xs text-slate-600 dark:text-slate-500 ml-7">
