  ...
  if (error) {
    return (
      <div className="text-center py-24">
        ...
        <p className="text-slate-600 dark:text-slate-500 text-sm mt-1">{(error as any)?.data?.message || 'Please try again later'}</p>
      </div>
    );
  }
  ...
  return (
    <div className="space-y-4 sm:space-y-8">
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-3 sm:gap-4">
        <div>
          <h2 className="text-xl sm:text-2xl font-bold text-slate-900 dark:text-white">Referral Program</h2>
      ...
      <div className="grid grid-cols-2 md:grid-cols-3 gap-3 sm:gap-6">
        <div className="col-span-2 md:col-span-1 bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 p-4 sm:p-6 rounded-xl sm:rounded-2xl relative overflow-hidden group">
          ...
          <p className="text-[10px] sm:text-xs text-slate-600 dark:text-slate-500 mt-1 sm:mt-2">
        ...
        <div className="bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 p-4 sm:p-6 rounded-xl sm:rounded-2xl relative overflow-hidden group">
          ...
          <p className="text-slate-500 dark:text-slate-400 font-medium mb-1 text-xs sm:text-base">Total Referrals</p>
          <h3 className="text-xl sm:text-3xl font-bold text-slate-900 dark:text-white">{totalReferrals}</h3>
          <p className="text-[10px] sm:text-xs text-slate-600 dark:text-slate-500 mt-1 sm:mt-2">
        ...
        <div className="bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 p-4 sm:p-6 rounded-xl sm:rounded-2xl relative overflow-hidden group">
          ...
          <p className="text-slate-500 dark:text-slate-400 font-medium mb-1 text-xs sm:text-base">Total Commission</p>
          <h3 className="text-xl sm:text-3xl font-bold text-slate-900 dark:text-white">{totalCommissionRate}%</h3>
          <p className="text-[10px] sm:text-xs text-slate-600 dark:text-slate-500 mt-1 sm:mt-2">
            Across 7 levels
          </p>
        </div>
      ...
      <div className="bg-gradient-to-br from-slate-100 to-white dark:from-slate-900 dark:to-slate-800 border border-slate-300 dark:border-slate-700 rounded-xl sm:rounded-2xl p-4 sm:p-8 relative overflow-hidden">
         ...
         <div className="relative z-10 text-center md:text-left md:flex items-center justify-between gap-8">
            <div className="mb-4 md:mb-0">
                <h3 className="text-base sm:text-xl font-bold text-slate-900 dark:text-white mb-1 sm:mb-2">Your Unique Referral Link</h3>
      ...
      <div className="bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-xl sm:rounded-2xl overflow-hidden shadow-lg">
            ...
            <tbody className="divide-y divide-slate-200 dark:divide-slate-800 text-sm text-slate-600 dark:text-slate-300">
              {referrals.length === 0 ? (
                ...
                referrals.map((ref: any, idx: number) => (
                  <tr key={ref.id || idx} className="hover:bg-slate-100/50 dark:hover:bg-slate-800/50 transition-colors">
                    <td className="px-6 py-4 font-medium text-slate-900 dark:text-white flex items-center">
                      <div className="w-8 h-8 rounded-full bg-gradient-to-br from-slate-200 to-slate-300 dark:from-slate-700 dark:to-slate-800 border border-slate-300 dark:border-slate-600 flex items-center justify-center mr-3 text-xs font-bold text-slate-500 dark:text-slate-400 shadow-sm">
                        {(ref.user || '?').charAt(0).toUpperCase()}
                      </div>
                      {ref.user || 'Unknown'}
                    </td>
                    <td className="px-6 py-4 text-slate-500 dark:text-slate-400 text-xs uppercase font-bold tracking-wide">Level {ref.level || 1}</td>
                    <td className="px-6 py-4 text-slate-500 dark:text-slate-400">{ref.date ? formatDate(ref.date) : '-'}</td>
                    <td className="px-6 py-4">
                      <span className={`px-2.5 py-1 rounded-full text-xs font-bold flex items-center w-fit ${
                        ...
                        <span className={`w-1.5 h-1.5 rounded-full mr-1.5 ${ref.status === 'active' ? 'bg-green-500' : 'bg-slate-300 dark:bg-slate-500'}`}></span>
        ...
        <div className="sm:hidden divide-y divide-slate-200 dark:divide-slate-800">
          {referrals.length === 0 ? (
            ...
            referrals.map((ref: any, idx: number) => (
              <div key={ref.id || idx} className="p-3 hover:bg-slate-100/50 dark:hover:bg-slate-800/50 transition-colors">
                <div className="flex items-center justify-between">
                  <div className="flex items-center space-x-3 min-w-0">
                    <div className="w-8 h-8 rounded-full bg-gradient-to-br from-slate-200 to-slate-300 dark:from-slate-700 dark:to-slate-800 border border-slate-300 dark:border-slate-600 flex items-center justify-center flex-shrink-0 text-xs font-bold text-slate-500 dark:text-slate-400">
                      {(ref.user || '?').charAt(0).toUpperCase()}
                    </div>
                    <div className="min-w-0">
                      <p className="text-xs font-medium text-slate-900 dark:text-white truncate">{ref.user || 'Unknown'}</p>
                      <p className="text-[10px] text-slate-600 dark:text-slate-500">Level {ref.level || 1} &middot; {ref.date ? formatDate(ref.date) : '-'}</p>
                    </div>
                  </div>
                  <div className="text-right flex-shrink-0 ml-2">
                    ...
                    <span className={`inline-flex items-center px-2 py-0.5 rounded-full text-[10px] font-bold ${
                      ...
                      <span className={`w-1 h-1 rounded-full mr-1 ${ref.status === 'active' ? 'bg-green-500' : 'bg-slate-300 dark:bg-slate-500'}`}></span>
                      {ref.status ? ref.status.charAt(0).toUpperCase() + ref.status.slice(1) : 'Pending'}
                    </span>
                  </div>
                </div>
